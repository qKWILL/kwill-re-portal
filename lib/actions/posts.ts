...
function generateSlug(title: string) {
  return (title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || `post-${Date.now()}`) + `-${Date.now()}`
}

export async function savePost(data: PostFormData, status: 'draft' | 'published'): Promise<SavePostResult> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) redirect('/login')

  // Validate
  const schema = status === 'published' ? (publishSchemas[data.type] ?? draftSchema) : draftSchema
  ...
  if (data.id) {
    const { data: before } = await supabase.from('posts').select('*').eq('id', data.id).single()
    const { error } = await supabase.from('posts').update(payload).eq('id', data.id)
    if (error) return { success: false, errors: { _: error.message } }
    const { data: after } = await supabase.from('posts').select('*').eq('id', data.id).single()
    await supabase.from('audit_log').insert({
      table_name: 'posts', record_id: data.id, action: 'update',
    ...
    const { data: newPost, error } = await supabase
      ...
      .select('id')
      .single()
    if (error || !newPost) return { success: false, errors: { _: error?.message ?? 'Failed to create post' } }
    postId = newPost.id
    await supabase.from('audit_log').insert({
      table_name: 'posts', record_id: newPost.id, action: 'create',
      before: null, after: newPost, performed_by: user.id,
    })
  }
  ...
  if (status === 'published') {
    ...
    if (configMap.revalidation_url && configMap.webhook_secret) {
      const body = JSON.stringify({
        event: 'publish', table: 'posts', record_id: postId,
...
export async function deletePost(postId: string) {
  ...
  if (isAdmin) {
    ...
    await supabase.from('posts').update({ deleted_at: new Date().toISOString() })
      .eq('id', postId).eq('created_by', user.id).eq('status', 'draft')
  }

  await supabase.from('audit_log').insert({
    table_name: 'posts', record_id: postId, action: 'delete',
    before: null, after: null, performed_by: user.id,
  })
